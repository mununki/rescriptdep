...
let has_cycle graph start_module =
  ...
  let rec visit path module_name =
    if List.mem module_name path then
      (* Cycle detected *)
      Some (List.rev (module_name :: List.filter (fun m -> m = module_name || List.hd path <> m) path))
      ...
      try
        List.find_map (fun dep -> visit new_path dep) deps
      with Not_found ->
        None
    end
  in
...
let topological_sort graph =
  ...
  let rec visit module_name =
    if not (Hashtbl.mem visited module_name) then begin
      Hashtbl.add visited module_name true;
      let deps = get_dependencies graph module_name in
...
let create_subgraph graph modules =
  ...
  StringMap.filter
    (fun m _ -> StringSet.mem m module_set)
    graph
  |> StringMap.map
       (fun deps ->
          List.filter
            (fun dep -> StringSet.mem dep module_set)
            deps)
...
let trim_to_reachable graph roots =
  ...
  let rec mark_reachable m =
    if not (Hashtbl.mem reachable m) then begin
  ...
  let reachable_modules = 
    Hashtbl.fold (fun m _ acc -> m :: acc) reachable [] 
  in
    ...
    List.iter
      (fun w ->
         if not (Hashtbl.mem indices w) then begin
           strong_connect w;
           Hashtbl.replace lowlinks v (min (Hashtbl.find lowlinks v) (Hashtbl.find lowlinks w))
         end else if Hashtbl.mem on_stack w then
           Hashtbl.replace lowlinks v (min (Hashtbl.find lowlinks v) (Hashtbl.find indices w)))
      deps;
    ...
    if Hashtbl.find lowlinks v = Hashtbl.find indices v then begin
      ...
      if List.length scc > 1 || 
         (List.length scc = 1 && 
          ...
          List.mem v (get_dependencies graph v)) then
        sccs := scc :: !sccs
    end
  in
